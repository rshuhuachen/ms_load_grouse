...
#### model ####
brm_load_t_add_high_lms <- brm(LMS_min ~ scale(gerp_Lt_additive_cat5) + core + (1|site) + (1|born), data = pheno_load,
                               family = "zero_inflated_poisson",
                               prior = prior(normal(0,1), class = b),
                               cores =8, control = list(adapt_delta = 0.99, max_treedepth = 15),
                               iter = iter, thin = thin, warmup = warm)

save(brm_load_t_add_high_lms, file = "results/bayes_models/lms_gerp5_total_add_1mil_zi_29scaf.RData")
