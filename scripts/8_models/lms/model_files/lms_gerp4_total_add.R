...
#### model ####
brm_load_t_add_gerp4_lms <- brm(LMS_min ~ scale(gerp_Lt_additive_cat4) + core + (1|site) + (1|born), data = pheno_load,
                               family = "zero_inflated_poisson",
                               prior = prior(normal(0,1), class = b),
                               cores =8, control = list(adapt_delta = 0.99, max_treedepth = 15),
                               iter = iter, thin = thin, warmup = warm)

save(brm_load_t_add_gerp4_lms, file = "results/bayes_models/lms_gerp4_total_add_zi_29scaf.RData")
