...
#### Model
parameter_m <- paste0(gsub("_region", "", method), "_load_m_", gsub("region_", "", region))
parameter_e <- paste0(gsub("_region", "", method), "_load_e_", gsub("region_", "", region))
formula <- formula(paste0("LMS_min ~ scale(", parameter_e, ") + scale(", parameter_m, ") + core + (1|site) + (1|born)"))
