### load packages ####
pacman::p_load(tidyverse, ggpubr, extrafont, cowplot, data.table, tolerance)
...
ti_95_gerp <- normtol.int(draws_gerp$beta, alpha = 0.05, P = 0.95, side = 2)
ti_80_gerp <- normtol.int(draws_gerp$beta, alpha = 0.05, P = 0.8, side = 2)
  
sum_gerp <- draws_gerp %>%
  summarize(lower_95 = ti_95_gerp$`2-sided.lower`[1],
            upper_95 = ti_95_gerp$`2-sided.upper`[1],
            lower_80 = ti_80_gerp$`2-sided.lower`[1],
            upper_80 = ti_80_gerp$`2-sided.upper`[1],
            mean = mean(beta)) 
...
ti_95_high <- normtol.int(draws_high$beta, alpha = 0.05, P = 0.95, side = 2)
ti_80_high <- normtol.int(draws_high$beta, alpha = 0.05, P = 0.8, side = 2)

sum_high <- draws_high %>%
  summarize(lower_95 = ti_95_high$`2-sided.lower`[1],
            upper_95 = ti_95_high$`2-sided.upper`[1],
            lower_80 = ti_80_high$`2-sided.lower`[1],
            upper_80 = ti_80_high$`2-sided.upper`[1],
            mean = mean(beta)) 
...
sum_gerp_region <- gerp_regions %>%
  group_by(region) %>%
  summarize(lower_95 = normtol.int(beta, alpha = 0.05, P = 0.95, side = 2) %>% .[1,4],
            upper_95 = normtol.int(beta, alpha = 0.05, P = 0.95, side = 2) %>% .[1,5],
            lower_80 = normtol.int(beta, alpha = 0.05, P = 0.8, side = 2) %>% .[1,4],
            upper_80 = normtol.int(beta, alpha = 0.05, P = 0.8, side = 2) %>% .[1,5],
            mean = mean(beta))
...
ggplot(gerp_regions, aes(x = beta)) + 
 facet_wrap(~region, ncol=1, strip.position = "top", scales="free_y")+
  ...
  # scale_fill_manual(values =alpha(c("grey60", clr_gerp), 0.7)) + #
  # scale_color_manual(values =c("grey60", clr_gerp)) +
  geom_segment(data=sum_gerp_region, aes(x = lower_95, 
                              xend = upper_95, 
                              y = 0), col = "black", linewidth=1)+
  geom_segment(data=sum_gerp_region, aes(x = lower_80, 
                             xend = upper_80, 
                             y = 0), col = "black", linewidth=3)+
  geom_point(data=sum_gerp_region,aes(x = mean, y = 0), fill="white",  col = "black", shape=21, size = 6)+
  geom_vline(xintercept = 0, col = "#ca562c", linetype="longdash", linewidth=0.6)+
...
ggplot(high_regions, aes(x = beta)) + 
  ...
  # scale_fill_manual(values =alpha(c("grey60", clr_high), 0.7)) + #
  # scale_color_manual(values =c("grey60", clr_high)) +
   geom_segment(data=sum_high_region, aes(x = lower_95, 
                             xend = upper_95, 
                             y = 0), col = "black", linewidth=1)+
  geom_segment(data=sum_high_region, aes(x = lower_80, 
                             xend = upper_80, 
                             y = 0), col = "black", linewidth=3)+
  geom_point(data=sum_high_region,aes(x = mean, y = 0), fill="white",  col = "black", shape=21, size = 6)+
  geom_vline(xintercept = 0, col = "#ca562c", linetype="longdash", linewidth=0.6)+
  theme(panel.border = element_blank(),
        panel.grid = element_blank(),
        strip.text = element_text(margin = margin(0,0,10,0)),
...
png("plots/sup/sup_3_random_draws.png", height = 1000, width = 1000)
fig
dev.off()
