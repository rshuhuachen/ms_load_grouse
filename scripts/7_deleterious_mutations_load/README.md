...
To filter out only high impact (and moderate impact) mutations annotated by SnpEff, we used SnpSift. Genetic load is then calculated for high impact mutations only.
...
## Gene regions
As we are interested in the effect of mutations across different genetic regions, we next annotate deleterious mutatations with gene regions using the genome annotation in subdirectory `per_region`. First, we split the gff file in different gene regions, followed by the annotation of each deleterious mutation. Lastly, we calculate genetic load based on the subset of mutations in a specific gene regions (e.g. TSS, intron, exon, etc.).  

## Combine
Lastly, we combine all genetic load scores in one file, which is outputted in `output/load/all_loads_combined_da_nosex_29scaf.RData`. We also compare the different types of load and calculate allele frequencies of deleterious mutations.
