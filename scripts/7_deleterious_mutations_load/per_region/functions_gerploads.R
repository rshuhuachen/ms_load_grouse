...
calculate_load_gerp <- function(file){
  ...
  for( id in 11:(ncol(file))){
    ...
    potential_data <- subset(subset_id, subset_id[[11]] == "1/0" | subset_id[[11]] == "0/1")
    realized_data <- subset(subset_id, subset_id[[11]] == "1/1")
    total_data <- subset(subset_id, subset_id[[11]] == "1/1" | subset_id[[11]] == "1/0" | subset_id[[11]] == "0/1")
    ...
    df <- data.frame(id = colnames(file[id]),
                     ...
                     gerp5_Lt_additive = (potential_load_sum*0.5 + realized_load_sum) / n_genotyped)
    load[[id]] <- df
  }
